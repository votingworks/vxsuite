...
test('export CVRs to USB in deprecated VotingWorks format', async () => {
  await withPlustekScannerApp(
    {},
    async ({ apiClient, mockScanner, mockUsb, workspace }) => {
...
test('unconfiguring machine', async () => {
  await withPlustekScannerApp(
    {},
    async ({ apiClient, mockUsb, interpreter, workspace }) => {
      await configureApp(apiClient, mockUsb);
...
test('auth', async () => {
  ...
  await withPlustekScannerApp({}, async ({ apiClient, mockUsb, mockAuth }) => {
    await configureApp(apiClient, mockUsb);
