...
/**
 * @deprecated use `withPlustekScannerApp` instead
 */
export async function createPlustekScannerApp({
  ...
  const { app, apiClient, stopApp } = await createApp({
    interpreter,
    logger,
    mockAuth,
    mockUsb,
  ...
  return {
    ...
    stopApp: () => {
      ...
      void mockScanner.disconnect();
      stopApp();
    },
  };
}

export interface WithPlustekScannerAppOptions {
  delays?: Partial<Delays>;
  mockScannerOptions?: Partial<MockScannerClientOptions>;
  preconfiguredWorkspace?: Workspace;
}

export interface WithPlustekScannerAppCallbackParameter {
  apiClient: grout.Client<Api>;
  app: Application;
  mockAuth: InsertedSmartCardAuthApi;
  mockScanner: MockScannerClient;
...
export async function withPlustekScannerApp(
  options: WithPlustekScannerAppOptions,
  callback: (parameter: WithPlustekScannerAppCallbackParameter) => Promise<void>
): Promise<void> {
  const { stopApp, ...rest } = await createPlustekScannerApp(options);
  try {
    await callback(rest);
  } finally {
    stopApp();
