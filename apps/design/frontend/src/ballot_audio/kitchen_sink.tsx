...
export function BallotAudioScreen(): React.ReactNode {
  ...
  const currentString = React.useMemo(() => {
    ...
    const fullKey = subkey ? `${stringKey}.${subkey}` : stringKey;
    throw new Error(`expected TTS string default for ${fullKey}`);
...
type PathParams = BallotAudioPathParams & { ttsMode: TtsExportSource };
...
function StringInfoContestTitleYesNo(props: {
  ...
  const { ttsMode = 'text', electionId } = useParams<PathParams>();
...
function AudioEditor(props: { str: TtsStringDefault }) {
  const { str } = props;
  ...
  }, [ttsMode, stringKey, subkey, textEditorRef.current]);

  const jurisdictionId =
    api.getElectionInfo.useQuery(electionId).data?.jurisdictionId;
  ...
  const originalTtsStr = ttsEdits?.text || str.text;
  ...
  function setTtsMode(newTtsMode: TtsExportSource) {
    history.push(
      routes
        .election(electionId)
        .ballots.audio.manage(newTtsMode, assertDefined(stringKey), subkey)
    );
  }
...
function StringSnippet(props: { string: TtsStringDefault }) {
  ...
  return (
    <StringSnippetContainer
      ...
      to={routes
        .election(electionId)
        .ballots.audio.manage(ttsMode, string.key, string.subkey)}
      role="option"
    >
      {string.text}
    </StringSnippetContainer>
