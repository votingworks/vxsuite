...
test('ballots not finalized', async () => {
  ...
  mockUser(api, 'support_user', { UNFINALIZE_BALLOTS: true });
  ...
  screen.getByText('Ballots not finalized');
  ...
  expect(screen.queryButton(/unfinalize/i)).not.toBeInTheDocument();
...
test('approve button', async () => {
  ...
  mockApprovedAt(api, approvedAt);
  api.getBaseUrl.expectCallWith().resolves('foo.com');
...
for (const userType of nonSupportUserTypes) {
  test(`approve button omitted for ${userType}`, async () => {
    const api = createMockApiClient();
    mockUser(api, userType, { UNFINALIZE_BALLOTS: true });
    mockFinalizedAt(api, finalizedAt);
    mockApprovedAt(api, null);
    mockMainExports(api, { completedAt: mainExportsDoneAt });
    mockTestDecks(api, { completedAt: testDecksDoneAt });
...
test('unfinalize button omitted if UNFINALIZE_BALLOTS flag is off', async () => {
  const api = createMockApiClient();
  mockUser(api, 'organization_user', { UNFINALIZE_BALLOTS: false });
  mockFinalizedAt(api, finalizedAt);
  mockApprovedAt(api, null);
  mockMainExports(api, { completedAt: mainExportsDoneAt });
  mockTestDecks(api, { completedAt: testDecksDoneAt });
...
function mockUser(
  api: MockApiClient,
  type: User['type'],
  features?: Partial<Record<UserFeature, boolean>>
) {
  api.getUser.expectCallWith().resolves({ ...user, type });
  api.getUserFeatures.expectCallWith().resolves(features || {});
}
