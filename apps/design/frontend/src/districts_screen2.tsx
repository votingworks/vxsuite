...
export function DistrictsScreen2(): JSX.Element {
  const { electionId } = useParams<ElectionIdParams>();
  const districtsParamRoutes = electionParamRoutes.districts2;
...
function Content(props: { editing: boolean }): JSX.Element | null {
  ...
  const districtsRoutes = routes.election(electionId).districts2;
  const districtsParamRoutes = electionParamRoutes.districts2;
    ...
    <Viewport>
      <ListActionsRow>
        <Button
          variant="primary"
          icon="Add"
          disabled={someMutationIsLoading || !!ballotsFinalizedAt}
          onPress={() => {
...
function DistrictRow(props: {
  ...
  const { electionId } = useParams<ElectionIdParams>();
  const districtRoutes = routes.election(electionId).districts2;

  return (
    <DistrictRowContainer key={district.id}>
      <InputWithAudio
        audioScreenUrl={districtRoutes.audio({
