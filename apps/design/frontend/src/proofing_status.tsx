...
export function ProofingStatus(): React.ReactNode {
  ...
  const userFeatures = api.getUserFeatures.useQuery();
  ...
  const showApprove = user.data.type === 'support_user' && !approved;
  const showUnfinalize = !!userFeatures.data.UNFINALIZE_BALLOTS && finalized;
