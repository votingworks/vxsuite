...
function expectedEnglishBallotStrings(election: Election): UiStringsPackage {
  ...
  return {
    ...
    [LanguageCode.ENGLISH]: {
      ...
      pollingPlaceName: Object.fromEntries(
        (election.pollingPlaces || []).map(({ id, name }) => [id, name])
      ),
...
test('create/list/delete elections', async () => {
  ...
  auth.setLoggedInUser(nonVxOrganizationUser);
  ...
  auth.setLoggedInUser(supportUser);
...
test('registered voter counts are stored and retrieved for precincts and splits', async () => {
  const { apiClient, workspace, auth } = await setupApp({
    organizations,
    jurisdictions,
    users,
  });
...
test('cloneElection', async () => {
  ...
  const newElectionId = await apiClient.cloneElection({
    electionId: srcElectionId,
    destElectionId: 'election-clone-1',
  ...
  await expect(
    apiClient.cloneElection({
      electionId: srcElectionId,
      destElectionId: 'election-clone-2',
  ...
  await expect(
    apiClient.cloneElection({
      electionId: 'election-clone-2',
      destElectionId: 'election-clone-3',
      destJurisdictionId: nonVxJurisdiction.id,
    })
...
test('Election package management', async () => {
  const baseElectionDefinition =
    electionFamousNames2021Fixtures.readElectionDefinition();
  const { apiClient, workspace, fileStorageClient, auth, baseUrl } =
...
test('Election package and ballots export', async () => {
  ...
  const { apiClient, workspace, fileStorageClient, auth } = await setupApp({
    organizations,
    jurisdictions,
    users,
  });
...
test('export - polling places auto-generated when EDIT_POLLING_PLACES === false', async () => {
  const baseElection =
    electionFamousNames2021Fixtures.readElectionDefinition().election;

  const inElection: Election = {
    ...baseElection,
    ballotLayout: {
      ...baseElection.ballotLayout,
      paperSize: HmpbBallotPaperSize.Legal,
    },
    signature: { caption: 'foo', image: 'bar' },
  };
  ...
  const electionId = (
    await apiClient.loadElection({
      ...
      upload: {
        format: 'vxf',
        electionFileContents: JSON.stringify(inElection),
      },
    })
  ).unsafeUnwrap();
  ...
  const outElection = electionPackage.electionDefinition.election;
  ...
  const exportedPlaces = assertDefined(outElection.pollingPlaces);
  const exportedPlaceNamesEn =
    outElection.ballotStrings[LanguageCode.ENGLISH]?.[
      ElectionStringKey.POLLING_PLACE_NAME
    ];
  expect(exportedPlaceNamesEn).toEqual(
    Object.fromEntries(exportedPlaces.map(({ id, name }) => [id, name]))
  );
});
...
test('Export test decks', async () => {
  ...
  const { apiClient, fileStorageClient, workspace, auth } = await setupApp({
    organizations,
    jurisdictions,
    users,
  });
...
test.each([
  ...
  async ({ bmdPrintMode, shouldIncludeSummaryBallots }) => {
    ...
    const electionId = (
      await apiClient.loadElection({
        newId: 'test-bmd-print-mode-election',
        jurisdictionId: nonVxJurisdiction.id,
        upload: {
          format: 'vxf',
          electionFileContents: electionDefinition.electionData,
...
test('api call logging', async () => {
  const { apiClient, logger, auth } = await setupApp({
    organizations,
    jurisdictions,
    users,
  });
