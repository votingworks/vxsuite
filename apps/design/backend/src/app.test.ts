...
function expectedEnglishBallotStrings(
  election: Election,
  precincts?: Precinct[]
): UiStringsPackage {
  const expectedStrings = mergeUiStrings(election.ballotStrings, {
    [LanguageCode.ENGLISH]: {
      ...hmpbStringsCatalog,
      ...(precincts ? getUserDefinedHmpbStrings(precincts) : {}),
    },
  });
  // TODO figure out why this special case is needed. Is `ballotStrings.precinctName`
  // pre-translated?
  if (precincts) {
    expectedStrings[LanguageCode.ENGLISH]!['precinctName'] = Object.fromEntries(
      precincts.map(({ id, name }) => [id, name])
    );
  }
...
test('precinct split config options show up in ballotStrings', async () => {
  const baseElectionDefinition =
    electionFamousNames2021Fixtures.readElectionDefinition();
  // Without mocking all the translations some ballot styles for non-English languages don't fit on a letter
  // page for this election. To get around this we use legal paper size for the purposes of this test.
  ...
  // Election fixtures don't have precincts with splits, so add one here
  let electionRecord = await apiClient.getElection({ electionId });
  const precinctToAdd: Precinct = {
    id: 'precinct-with-split-01',
    name: 'Northwest Lincoln',
    splits: [
      {
        districtIds: [baseElectionDefinition.election.districts[0].id],
        id: 'split-01',
        name: 'Split 1',
  ...
  const precinctsWithSplit: Precinct[] = [
    ...electionRecord.precincts,
    precinctToAdd,
  ];

  await apiClient.updatePrecincts({
    electionId,
    precincts: precinctsWithSplit,
  });
  ...
  electionRecord = await apiClient.getElection({ electionId });

  const { election: appElection, precincts: appPrecincts } = electionRecord;
  ...
  expect(electionDefinition.election).toEqual({
    ...
    precincts: [
      ...electionWithLegalPaper.precincts,
      { id: precinctToAdd.id, name: precinctToAdd.name },
    ],
    ...
    ballotStrings: expect.objectContaining({
      ...
      ...expectedEnglishBallotStrings(appElection, appPrecincts),
    }),
  });
});
