...
import { forEachUiString, setUiStringAudioIds, textToAudioIds } from './utils';
...
export function generateAudioIdsAndClips({
  ...
  function populateUiStringAudioIds({
    ...
    const audioIds = textToAudioIds(languageCode, stringInLanguage);
    setUiStringAudioIds(
      uiStringAudioIds,
      languageCode,
      stringKey,
      audioIds.map(({ audioId }) => audioId)
    );
