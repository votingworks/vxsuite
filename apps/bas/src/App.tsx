...
const App: React.FC<Props> = ({ card }) => {
  ...
  ] = useStateAndLocalStorage('singlePrecinctMode', z.boolean(), false)
  const [election, setElection] = useStateAndLocalStorage(
    'election',
    schema.Election
  )
  const [isLoadingElection, setIsLoadingElection] = useState(false)
  const [precinctId, setPrecinctId] = useStateAndLocalStorage(
    'precinctId',
    z.string()
  )
  const [ballotStyleId, setBallotStyleId] = useState<string>()
  const [partyId, setPartyId] = useStateAndLocalStorage('partyId', z.string())
