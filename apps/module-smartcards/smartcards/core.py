...
def get_mock(): # pragma: no cover this is just for testing
    if CardInterface == RealInstance:
        ...
        short_value = MockInstance.short_value.decode('utf-8') if MockInstance.short_value else None
        long_value = MockInstance.long_value.decode('utf-8') if MockInstance.long_value else None

        return json.dumps({
            'enabled': True,
            'has_card': MockInstance.has_card,
