...
async function createBackupDir(
  ...
  const dbHash = await writeFileWithHash(
  ...
  for (const image of images) {
    ...
    const hash = await writeFileWithHash(fullPath, image.content);
    files.push({
      path: imagePath,
      sha256: hash,
  ...
  await writeFile(join(backupDir, MANIFEST_FILENAME), manifestJson, 'utf-8');
  await writeFile(
 ...
 */
async function addBallotImageToWorkspace(
...
describe('list command', () => {
  ...
  test('lists backups with metadata', async () => {
    ...
    await createBackupDir(mountPoint, 'election-2026-general', {
  ...
  test('lists multiple backups sorted by date', async () => {
    ...
    await createBackupDir(mountPoint, 'election-old', {
      electionTitle: 'Old Election',
      createdAt: '2025-01-01T00:00:00.000Z',
    });
    await createBackupDir(mountPoint, 'election-new', {
...
describe('validate command', () => {
  ...
  test('validates a good backup', async () => {
    ...
    const mountPoint = join(tmpDir, 'drive');
    await mkdir(mountPoint, { recursive: true });
    const backupDir = await createBackupDir(mountPoint, 'election-good', {
  ...
  test('reports validation failure for tampered manifest', async () => {
    ...
    const mountPoint = join(tmpDir, 'drive');
    await mkdir(mountPoint, { recursive: true });
    const backupDir = await createBackupDir(mountPoint, 'election-tampered');
...
describe('backup command', () => {
  ...
  test('performs a successful backup, reading election from database', async () => {
    ...
    const backupDirs = (await ignoreMissing(readdir(backupRootPath))) ?? [];
    ...
    expect(
      (await stat(join(backupDir, MANIFEST_SIGNATURE_FILENAME))).isFile()
    ).toEqual(true);
    ...
    const manifest: BackupManifest = JSON.parse(
      await readFile(join(backupDir, MANIFEST_FILENAME), 'utf-8')
    );
  ...
  test('performs backup with ballot images', async () => {
    ...
    await addBallotImageToWorkspace(
      workspace,
      'image-data-1',
      'cvr-1',
      'front'
    );
    await addBallotImageToWorkspace(workspace, 'image-data-2', 'cvr-1', 'back');
    ...
    const backupRootPath = join(mountPoint, BACKUP_ROOT_DIR);
    const backupDirs = await readdir(backupRootPath);
    const backupDir = join(backupRootPath, assertDefined(backupDirs[0]));
    const manifest: BackupManifest = JSON.parse(
      await readFile(join(backupDir, MANIFEST_FILENAME), 'utf-8')
    );
...
describe('restore command', () => {
  ...
  test('errors when multiple backups on drive', async () => {
    ...
    const mountPoint = join(tmpDir, 'usb');
    await mkdir(mountPoint, { recursive: true });
    await createBackupDir(mountPoint, 'backup-a', {
      electionTitle: 'Election A',
    });
    await createBackupDir(mountPoint, 'backup-b', {
      electionTitle: 'Election B',
    });
  ...
  test('auto-detects single backup on drive', async () => {
    ...
    const mountPoint = join(tmpDir, 'usb');
    await mkdir(mountPoint, { recursive: true });
    await createBackupDir(mountPoint, 'election-restore', {
  ...
  test('performs restore with images', async () => {
    ...
    const mountPoint = join(tmpDir, 'usb');
    await mkdir(mountPoint, { recursive: true });
    await createBackupDir(mountPoint, 'election-images', {
...
describe('SIGINT cancellation', () => {
  test('backup surfaces non-cancellation errors', async () => {
    ...
    expect(code).toEqual(1);
    expect(io.stderr.toString()).toContain(
      'Backup drive has insufficient space'
    );
  });
  ...
  test('restore surfaces non-cancellation errors', async () => {
    ...
    const mountPoint = join(tmpDir, 'usb');
    await mkdir(mountPoint, { recursive: true });
    await createBackupDir(mountPoint, 'election-err');
    const workspace = join(tmpDir, 'restore-workspace');
    await mkdir(workspace, { recursive: true });
  ...
  test('restore returns 130 when cancelled', async () => {
    ...
    const mountPoint = join(tmpDir, 'usb');
    await mkdir(mountPoint, { recursive: true });
    await createBackupDir(mountPoint, 'election-cancel');
...
describe('error resilience', () => {
  test('handles corrupt manifest JSON gracefully', async () => {
    ...
    const mountPoint = join(tmpDir, 'drive');
    await mkdir(mountPoint, { recursive: true });
    const backupDir = await createBackupDir(mountPoint, 'election-corrupt');
  ...
  test('handles missing manifest signature file', async () => {
    ...
    await mkdir(backupDir, { recursive: true });
    await writeFile(join(backupDir, MANIFEST_FILENAME), '{}', 'utf-8');
  ...
  test('list handles directories with corrupt manifests gracefully', async () => {
    ...
    await writeFile(join(corruptDir, MANIFEST_FILENAME), 'bad json', 'utf-8');

    await createBackupDir(mountPoint, 'good-backup', {
