...
import {
  BackupStopReason,
  performBackup,
  listBackups as listBackupsFromDrive,
} from './backup';
...
export class BackupManager {
  ...
  private currentOperationPromise?: Promise<void>;
  ...
  async refreshDriveCache(): Promise<void> {
    ...
    for (const drive of drives) {
      for (const partition of drive.partitions) {
        ...
        const backupRootStat = await stat(backupRootPath).catch(
          () => undefined
        );
        const isBackupDrive = backupRootStat?.isDirectory() === true;
        ...
        if (isBackupDrive) {
          newBackups.set(mountPoint, await listBackupsFromDrive(mountPoint));
        }
      }
    }
   ...
   */
  async getBackupDrives(): Promise<BackupDriveInfo[]> {
    await this.refreshDriveCacheIfStale();
   ...
   */
  private async refreshDriveCacheIfStale(): Promise<void> {
    const drives = this.multiUsbDrive.getDrives();
  ...
  async designateBackupDrive(driveDevPath: string): Promise<void> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'EACCES') {
        ...
        await mkdir(join(newMountPoint, BACKUP_ROOT_DIR), { recursive: true });
        debug(
   ...
   *
   * Returns immediately; backup runs in background and callers should rely on
   * {@link getStatus} updates to determine success/failure. The only case that
   * returns an `Err` from this method is when another backup is already
   * running.
   */
  startBackup(
    ...
    if (this.status.type === 'running') {
      return err({ type: 'alreadyRunning' });
    }
    ...
    this.currentOperationPromise = this.runBackup({
      trigger,
      backupDriveMountPoint,
      machineId,
      softwareVersion,
    }).finally(() => {
      this.currentOperationPromise = undefined;
    });

    return ok();
  }

  async waitForCurrentOperation(): Promise<void> {
  ...
  private async runBackup({
    ...
    let backupResult: Result<void, BackupStopReason>;
    try {
      backupResult = await performBackup({
      ...
      debug('backup threw unexpectedly: %s', errorMessage);
      return;
    }
    ...
    if (backupResult.isErr()) {
      ...
      const errorMessage = formatBackupStopReason(error);
      this.setStatus({
        type: 'error',
        trigger,
        error: errorMessage,
