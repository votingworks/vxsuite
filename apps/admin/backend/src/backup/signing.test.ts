...
describe('validateManifestSignature', () => {
  test('accepts a valid HMAC signature', () => {
    ...
    expect(validateManifestSignature(Buffer.from(json), sig)).toEqual(true);
  });

  test('rejects a tampered manifest', () => {
    const sig = signManifest('{"original":true}');
    expect(
      validateManifestSignature(Buffer.from('{"tampered":true}'), sig)
    ).toEqual(false);
  });
  ...
  test('rejects unknown signature format', () => {
    ...
    expect(validateManifestSignature(Buffer.from(json), sig)).toEqual(false);
  });
});
