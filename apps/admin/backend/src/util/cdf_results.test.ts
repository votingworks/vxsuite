...
test('buildElectionResultsReport emits PartyContest for straight-party contests', () => {
  ...
  const firstPartyId = election.parties[0].id;
  spResults.tallies[firstPartyId] = {
    partyId: firstPartyId,
    name: election.parties[0].fullName,
    tally: 7,
  };
  ...
  const partyContest = report.Election?.[0].Contest.find(
  ...
  const firstPartySelection = partyContest?.ContestSelection.find((s) =>
    s.PartyIds[0].endsWith(firstPartyId)
  ) as ResultsReporting.PartySelection | undefined;
