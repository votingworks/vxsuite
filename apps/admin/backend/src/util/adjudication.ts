...
/**
 * Builds a {@link BallotPageImage} from pre-loaded image metadata and optional
 * layout.
 */
function buildBallotPageImage(
...
export async function getBallotImages({
  ...
  const [front, back] = await mapSheet(
    ...
    async ({ image, layout }): Promise<BallotPageImage> => {
      const metadata = (await loadImageMetadata(image)).ok();
      const imageUrl = metadata
        ? `data:${metadata.type};base64,${image.toString('base64')}`
        : undefined;
      return buildBallotPageImage(metadata, layout, imageUrl);
    }
  );
...
export async function getBallotImageMetadata({
  ...
  const [front, back] = await mapSheet(
    ...
    async ({ image, layout }, side): Promise<BallotPageImage> => {
      const metadata = (await loadImageMetadata(image)).ok();
      return buildBallotPageImage(metadata, layout, buildImageUrl(side));
    }
  );
