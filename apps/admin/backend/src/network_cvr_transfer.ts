...
async function readTransferManifest(
  ...
  /* istanbul ignore next - manifests are only written by us */
  return parseResult.isOk() ? parseResult.ok() : undefined;
}
...
export async function receiveCvrTransferUpload(
  ...
  const electionId = store.getCurrentElectionId();
  /* istanbul ignore next - the transfer can't start while unconfigured */
  if (!electionId) {
    return err('transfer not started');
  }
  const { electionDefinition } = assertDefined(store.getElection(electionId));
  ...
  // Land the file set in a scratch directory so it can be read with the
  // same reader as USB imports; it's absorbed into staging before returning.
  const scratchDirectoryPath = path.join(
    transferDirectoryPath,
    'incoming',
    castVoteRecordId
  );
  try {
    await fs.rm(scratchDirectoryPath, { recursive: true, force: true });
    await fs.mkdir(scratchDirectoryPath, { recursive: true });
    for (const entry of entries) {
      await fs.writeFile(
        path.join(scratchDirectoryPath, entry.name),
    ...
    if (prepared.needsAdjudication && prepared.referencedFiles) {
      images = [];
...
export async function finishCvrTransfer(
  ...
  const { store } = workspace;
  if (!isSafeId(input.machineId) || !isSafeId(input.batchId)) {
    return err({ type: 'transfer-not-found' });
  }
  const electionId = store.getCurrentElectionId();
  if (!electionId) {
    return err({ type: 'transfer-not-found' });
