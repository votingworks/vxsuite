...
test('exporting PDF', async () => {
  ...
  const { resolve } = apiMock.expectExportTallyReportPdf({
    expectCallWith: {
      path: `test-mount-point/franklin-county_lincoln-municipal-general-election_${electionDefinition.ballotHash.slice(
        0,
        10
      )}/reports/unofficial-tally-reports-by-voting-method__2023-09-06_21-45-08.pdf`,
      ...MOCK_REPORT_SPEC,
    },
    returnValue: ok([]),
    deferred: true,
  ...
  await vi.waitFor(() => screen.getByText('Saving Tally Report'));
  resolve();
  await vi.waitFor(() => screen.getByText('Tally Report Saved'));
});
  ...
  const { resolve } = apiMock.expectExportTallyReportCsv({
    expectCallWith: {
      path: `test-mount-point/franklin-county_lincoln-municipal-general-election_${electionDefinition.ballotHash.slice(
        0,
        10
      )}/reports/unofficial-tally-report-by-voting-method__2023-09-06_21-45-08.csv`,
...
test('when full election report - allows CDF export and includes signature lines', async () => {
  ...
  const { resolve } = apiMock.expectExportCdfReport({
    expectCallWith: {
      path: `test-mount-point/franklin-county_lincoln-municipal-general-election_${electionDefinition.ballotHash.slice(
        0,
        10
      )}/reports/unofficial-cdf-election-results-report__2023-09-06_21-45-08.json`,
    },
    returnValue: ok([]),
    deferred: true,
  });
  userEvent.click(within(modal).getButton('Save'));
  await vi.waitFor(() =>
    screen.getByText('Saving CDF Election Results Report')
  );
  resolve();
  await vi.waitFor(() => screen.getByText('CDF Election Results Report Saved'));
});
