...
function makeMockOptionLayout(
  definition: CandidateContestOption
): BallotPageContestOptionLayout {
  return {
    definition,
    bounds: definition.isWriteIn
      ? { x: 400, y: 200, width: 400, height: 200 }
      : { x: 200, y: 100, width: 50, height: 30 },
    target: definition.isWriteIn
      ? {
...
function renderScreen(
  ...
  cvrId: Id,
  {
    ...
    onAdjudicateCvrContest = vi.fn().mockResolvedValue(undefined),
  }: {
...
describe('hmpb write-in adjudication', () => {
  ...
  test('hmpb write-in can be adjudicated as invalid', async () => {
    ...
    const { onClose, onAdjudicateCvrContest } = renderScreen(data, cvrId, {
    ...
    expect(onAdjudicateCvrContest).toHaveBeenCalledWith(adjudicatedCvrContest);
  });
...
describe('vote adjudication', () => {
  ...
  test('yesno contest can have votes adjudicated', async () => {
    ...
    await waitFor(() => expect(onClose).toHaveBeenCalled());
    expect(onAdjudicateCvrContest).toHaveBeenCalledWith(expectedAdjudication);
  });
});
...
describe('marginal mark adjudication', () => {
  ...
  test('hmpb ballot can have marginally marked official option adjudicated', async () => {
    ...
    const expectedAdjudication = formAdjudicatedCvrContest(cvrId, {
      kangaroo: { type: 'candidate-option', hasVote: true },
    });
