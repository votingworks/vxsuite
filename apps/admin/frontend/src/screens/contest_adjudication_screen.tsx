...
export function ContestAdjudicationScreen(): JSX.Element {
  ...
  const [marginalMarks, setMarginalMarks] =
  ...
  useEffect(() => {
    if (
      ...
      setMarginalMarks(newMarginalMarkStatusByOptionId);
  ...
  const allMarginalMarksAdjudicated = !Object.values(marginalMarks).some(
          ...
          {!isVoteStateReady ? (
            ...
            <BallotOptionButtonList ref={ballotOptionListRef}>
              {officialOptions.map((officialOption) => {
                ...
                const marginalMarkStatus = marginalMarks?.[officialOption.id];
                return (
                  <BallotOptionButton
                    ...
                    marginalMarkStatus={marginalMarkStatus}
                    onClickFlag={() => {
                      console.log(officialOption.id, 'clicked flag');
                      setFocusedOptionId(officialOption.id);
                    }}
                    onDismissFlag={() => {
                      resolveMarginalMark(officialOption.id);
                      setFocusedOptionId(undefined);
                    }}
                    isFocused={focusedOptionId === officialOption.id}
                  />
                );
              })}
              {writeInOptionIds.map((optionId) => {
                ...
                const marginalMarkStatus = marginalMarks?.[optionId];
