...
export function ContestAdjudicationScreen(): JSX.Element {
  ...
  const firstRequiringAdjudicationId = isStateReady
    ? options.find(
        (o) =>
          o.marginalMarkStatus === 'pending' ||
          (o.isWriteIn && isPendingWriteIn(o.writeInAdjudicationStatus))
      )?.optionId
    : undefined;
      ...
      <Main flexRow data-testid={`transcribe:${currentCvrId}`}>
        ...
        <AdjudicationPanel>
          ...
          <BallotFooter>
            ...
            <BallotNavigation>
              <PrimaryNavButton
                disabled={!allAdjudicationsCompleted || !isModified}
                icon="Done"
                onPress={saveAndNext}
                variant="primary"
              >
