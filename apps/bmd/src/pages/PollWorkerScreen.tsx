...
interface Props {
  activateCardlessBallot: (precinctId: string, ballotStyleId?: string) => void
...
const PollWorkerScreen: React.FC<Props> = ({
  activateCardlessBallot,
  resetCardlessBallot,
  appPrecinct,
  ballotsPrintedCount,
  precinctId = appPrecinct.kind === PrecinctSelectionKind.SinglePrecinct
  ...
  const precinctBallotStyles = precinctId
        ...
        <Main>
          <MainChild center narrow>
            <Prose>
              <h1>
                {appPrecinct.kind === PrecinctSelectionKind.AllPrecincts
                  ? `Voter session activated: ${ballotStyleId} @ ${activationPrecinctName}`
                    ...
                    <ButtonList data-testid="precincts">
                      {election.precincts.map((precinct) => (
                        <Button
                          fullWidth
                          key={precinct.id}
                          onPress={() => activateCardlessBallot(precinct.id)}
                          primary={precinctId === precinct.id}
                        >
                          {precinct.name}
                        </Button>
                      ))}
                    ...
                    <ButtonList data-testid="ballot-styles">
                      {precinctBallotStyles.map((bs) => (
                        <Button
                          ...
                          key={bs.id}
                          onPress={() =>
                            activateCardlessBallot(precinctId, bs.id)
                          }
                        >
                          {bs.id}
                        </Button>
