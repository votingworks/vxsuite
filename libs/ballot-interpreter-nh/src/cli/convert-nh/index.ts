...
function usage(out: NodeJS.WritableStream): void {
  out.write(
    `usage: convert-nh <definition.xml> <front-ballot.jpg> <back-ballot.jpg> -o <output.json>\n`
  );
}
...
export async function main(args: readonly string[]): Promise<number> {
  ...
  if (parseResult.isErr()) {
    process.stderr.write(`error: ${parseResult.err().message}\n`);
    return 1;
  }
  ...
  if (convertResult.isErr()) {
    process.stderr.write(`error: ${convertResult.err().message}\n`);
    return 1;
  }

  await fs.writeFile(outputPath, JSON.stringify(convertResult.ok(), null, 2));
