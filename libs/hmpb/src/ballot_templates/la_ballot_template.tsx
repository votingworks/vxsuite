...
function CandidateContest({
  ...
  const voteForText = {
    ...
    11: hmpbStrings.hmpbVoteFor11,
    12: hmpbStrings.hmpbVoteFor12,
    13: hmpbStrings.hmpbVoteFor13,
    14: hmpbStrings.hmpbVoteFor14,
  }[contest.seats];
  if (!voteForText) {
    throw new Error(
...
const PresidentialCandidateGridCell = styled.li`
  /* We want internal borders between cells only */
...
function PresidentialCandidateContest({
  election,
  contest,
}: {
  election: Election;
  ...
  const candidateBallotStrings = assertDefined(
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    <div>
                      <strong>{ballotStrings.presidentialCandidateName}</strong>
                  ...
                  <div style={{ fontSize: '0.7rem', marginTop: '0.625rem' }}>
                    <strong>Electors:</strong>
                    <ul>
                      {ballotStrings.electors.map((elector) => (
                        <li key={elector}>{elector}</li>
                      ))}
                    </ul>
                  </div>
                </div>
...
async function BallotPageContent(
  ...
  let contestSections = iter(contests)
    .partition((contest) => contest.type === 'candidate')
    .filter((section) => section.length > 0);
  if (contestSections.length > 1) {
    contestSections = [
...
export const laBallotTemplate: BallotPageTemplate<BaseBallotProps> = {
