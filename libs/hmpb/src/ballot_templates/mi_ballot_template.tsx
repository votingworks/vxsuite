...
async function PrimaryBallotPageContent(
  ...
  const contestSections = [
    ...
    .map((section) => ({
      isPartisan: 'isPartisan' in section ? section.isPartisan : false,
  ...
  const [partisanSectionHeaderMeasurements] = await scratchpad.measureElements(
    ...
    >
      <div
        style={{
          width: `${
            columnWidthPx * numPartisanColumns +
            horizontalGapPx * (numPartisanColumns - 1)
          }px`,
        }}
        className="wrapper"
      >
  ...
  const sectionsElement = (
    <div style={{ display: 'flex', gap: `${horizontalGapPx}px` }}>
      ...
      <div style={{ width: `${nonPartisanWidthPercent}%` }}>
        <div />
