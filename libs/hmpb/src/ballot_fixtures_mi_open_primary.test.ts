...
import { miOpenPrimaryFixtures } from './ballot_fixtures';
...
// run `pnpm generate-fixtures --mi-open-primary` if this test fails
test('MI open primary fixtures', async () => {
  const fixtures = miOpenPrimaryFixtures;
  const generated = await miOpenPrimaryFixtures.generate(rendererPool);
