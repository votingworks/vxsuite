...
/**
 * Optional language context for rendering ballots in specific languages.
 * When provided, the ballot will be rendered with translations,
 * ensuring accurate page break calculations for non-English ballots.
 */
...
async function measureBallotHeight(
  ...
  // Wrap with language context if provided, ensuring text is rendered
  // in the correct language for accurate height measurement
  const ballot = languageContext ? (
    <BackendLanguageContextProvider
      currentLanguageCode={languageContext.languageCode}
      uiStringsPackage={languageContext.uiStringsPackage}
    >
      {ballotElement}
    </BackendLanguageContextProvider>
  ) : (
