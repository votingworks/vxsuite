...
function createFakeSpawnResult(): ReturnType<typeof spawn> {
  return {
    stdout: { on: vi.fn() },
    stderr: { on: vi.fn() },
    on: vi.fn(),
...
describe('AvahiService.advertiseService', () => {
  afterEach(() => {
    vi.clearAllMocks();
    AvahiService.stopAdvertisedService();
  });

  it('spawns the intermediate avahi-publish-service script with default service type', () => {
    mockSpawn.mockReturnValue(createFakeSpawnResult());
    AvahiService.advertiseService('test-service', 1234);
  ...
  it('spawns with custom service type', () => {
    mockSpawn.mockReturnValue(createFakeSpawnResult());
    AvahiService.advertiseService('test-service', 1234, '_vxadmin._tcp');
    expect(mockSpawn).toHaveBeenCalledWith('bash', [
      expect.stringContaining('avahi-publish-service'),
      'test-service',
      '1234',
...
describe('AvahiService.stopAdvertisedService', () => {
  ...
  it('kills all running processes without name', () => {
    const fakeProcess = { kill: vi.fn() } as unknown as ReturnType<
      typeof spawn
    >;
    const fakeProcess2 = { kill: vi.fn() } as unknown as ReturnType<
...
describe('AvahiService.discoverServices', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  it('returns parsed services on success with default service type', async () => {
    const stdout =
    ...
    expect(mockExecFileFn).toHaveBeenCalledWith('bash', [
      expect.stringContaining('avahi-browse'),
      '_http._tcp',
    ]);
  });

  it('passes custom service type to avahi-browse', async () => {
    mockExecFileFn.mockResolvedValue({ stdout: '', stderr: '' });
    await AvahiService.discoverServices('_vxadmin._tcp');
    expect(mockExecFileFn).toHaveBeenCalledWith('bash', [
      expect.stringContaining('avahi-browse'),
      '_vxadmin._tcp',
    ]);
  ...
  it('ignores lines that are not resolved services or incomplete', async () => {
    ...
    mockExecFileFn.mockResolvedValue({ stdout, stderr: '' });
    const result = await AvahiService.discoverServices();
...
describe('AvahiService.parseBrowseOutput', () => {
  it('handles empty output', () => {
    expect(AvahiService.parseBrowseOutput('')).toEqual([]);
  });
});
