...
/**
 * `golden_payloads.json` was produced by the TypeScript implementation that
 * used to live in this package, captured immediately before it was deleted.
 * They are the wire format: every ballot in the field was printed by the code
 * that produced them, so a change here is a change to what scanners must read,
