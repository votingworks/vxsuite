...
import { JsBitReader } from './bit_reader';
import * as addon from '../addon/bits/bit_reader';

describe.each([['JsBitReader', JsBitReader], ['BitReader (Native)', addon.BitReaderNative]])("%s", (_, R) => {
  test('reads bits in little-endian order', () => {
    const reader = new R(Uint8Array.of(0b11010000));
  ...
  test('reads booleans as true iff the bit is set', () => {
    const reader = new R(Uint8Array.of(0b10000000));
  ...
  test('can read as long as there is data to read', () => {
    expect(new R(Uint8Array.of()).canRead()).toEqual(false);

    const reader = new R(Uint8Array.of(0));
  ...
  test('can read uints with various options', () => {
    const bits = new R(Uint8Array.of(0xff, 0x0f));
  ...
  test('cannot read a uint specifying both `max` and `size` options', () => {
    expect(() => {
      ...
      new R(Uint8Array.of()).readUint(options);
  ...
  test('can skip given bits', () => {
    const bits = new R(Uint8Array.of(0b10101010));
  ...
  test('can skip given bytes', () => {
    const bits = new R(Uint8Array.of(1, 2, 3));
  ...
  test('can skip uints of arbitrary size', () => {
    const bits = new R(Uint8Array.of(0b11001100));
