...
use super::args::{
    get_election_definition_from_arg, get_image_source_from_arg, get_path_from_arg_opt,
};
...
pub fn interpret(mut cx: FunctionContext) -> JsResult<JsObject> {
    ...
    let side_a_image_or_path = get_image_source_from_arg(&mut cx, 1)?;
    let side_b_image_or_path = get_image_source_from_arg(&mut cx, 2)?;
    ...
    let (side_a_image, side_b_image) = rayon::join(
        || side_a_image_or_path.to_luma8(),
        || side_b_image_or_path.to_luma8(),
    );
