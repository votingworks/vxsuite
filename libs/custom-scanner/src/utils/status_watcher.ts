...
const debug = rootDebug.extend('waitForStatus');
...
/**
 * Polls the status of the scanner until the predicate returns true.
 */
export async function waitForStatus(
  scanner: CustomScanner,
  predicate: (status: ScannerStatus) => boolean,
  { timeout }: { timeout?: number } = {}
): Promise<Result<Optional<ScannerStatus>, ErrorCode>> {
  debug('starting');
  const statusWatcher = watchStatus(scanner);

  if (typeof timeout === 'number') {
    void sleep(timeout).then(() => {
      debug('status watcher timed out');
      statusWatcher.stop();
    });
  }

  for await (const statusResult of statusWatcher) {
    if (statusResult.isErr()) {
      debug('status watcher error: %o', statusResult.err());
      statusWatcher.stop();
      return statusResult;
    }

    const status = statusResult.ok();
    debug('status watcher status: %o', status);

    if (predicate(status)) {
      debug('status watcher predicate matched');
      statusWatcher.stop();
      return statusResult;
    }
  }
