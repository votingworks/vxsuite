...
import { encodeTallyEntries, encodePrecinctBitmap } from '@votingworks/utils';
...
const sampleBitmap = 'mockBitmap';

vi.mock(
  '@votingworks/utils',
  async (importActual): Promise<typeof import('@votingworks/utils')> => ({
    ...(await importActual<typeof import('@votingworks/utils')>()),
    encodeTallyEntries: vi
      .fn()
      .mockImplementation((args: { numPages?: number }) => {
        const numPages = args?.numPages ?? 1;
        const tally = 'a'.repeat(500);
        // Split tally into numPages equal parts
        const partLength = Math.ceil(tally.length / numPages);
        const parts = [];
        for (let i = 0; i < numPages; i += 1) {
          parts.push(tally.slice(i * partLength, (i + 1) * partLength));
        }
        return parts;
      }),
    encodePrecinctBitmap: vi.fn().mockReturnValue('mockBitmap'),
    getPrecinctIdsFromBitmap: vi.fn().mockReturnValue([]),
  })
);
...
test.each<{ isLiveMode: boolean }>([
  ...
  async ({ isLiveMode }) => {
    ...
    expect(encodeTallyEntries).toHaveBeenCalledTimes(1);
...
test.each<{ isLiveMode: boolean }>([
  ...
  async ({ isLiveMode }) => {
    ...
    expect(encodeTallyEntries).toHaveBeenCalledTimes(3);
...
test('If it is impossible to fit the signed quick results reporting URL within the max length even when broken into the maximum number of parts, an error is thrown', async () => {
  ...
  expect(encodeTallyEntries).toHaveBeenCalledTimes(25);
}, 20000);
...
test('generateSignedQuickResultsReportingUrl works for reporting polls open status - live all precincts', async () => {
  ...
  expect(encodeTallyEntries).toHaveBeenCalledTimes(0);
  expect(signedQuickResultsReportingUrl).toMatch(
    /^https:\/\/example.com\?p=.*&s=[^&]+&c=[^&]+$/
  );
...
test('generateSignedQuickResultsReportingUrl works for reporting polls open status - test single precincts', async () => {
  ...
  expect(signedQuickResultsReportingUrl).toContain('open_polls');
  // In qr3, the bitmap is in field 5 instead of precinctId
  expect(signedQuickResultsReportingUrl).toContain(sampleBitmap);
});
...
test('decodeQuickResultsMessage throws error when given invalid payload', () => {
  expect(() => {
    decodeQuickResultsMessage('invalidPayload', election);
...
test('decodeQuickResultsMessage rejects invalid numPages, pageIndex, ballotCount, and votingType values', () => {
  ...
  // Test qr2 format as well for backwards compatibility
  ...
  // numPages must be >= 1
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ numPages: '0' }), election)
  ).toThrow('Invalid number of pages format');
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ numPages: '-1' }), election)
  ).toThrow('Invalid number of pages format');

  // pageIndex must be >= 0
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ pageIndex: '-1' }), election)
  ).toThrow('Invalid page index format');

  // ballotCount must be a valid non-negative integer
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ ballotCount: 'abc' }), election)
  ).toThrow('Invalid ballot count format');
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ ballotCount: '-1' }), election)
  ).toThrow('Invalid ballot count format');

  // votingType must be a valid digit mapping to a known type
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ votingType: 'abc' }), election)
  ).toThrow('Invalid voting type format');
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ votingType: '99' }), election)
  ).toThrow('Invalid voting type format');
  expect(() =>
    decodeQuickResultsMessage(buildPayload({ votingType: '-1' }), election)
  ).toThrow('Invalid voting type format');
});

test('encodeQuickResultsMessage and decodeQuickResultsMessage handle proper payloads with bitmap', () => {
  const decoded = decodeQuickResultsMessage(
    constructPrefixedMessage(
      ...
      encodeQuickResultsMessage({
        ...
        primaryMessage: 'sampleCompressedTally',
        encodedPrecinctBitmap: 'mockBitmapData',
        numPages: 1,
        pageIndex: 0,
        ballotCount: 42,
        votingType: 'election_day',
...
test('encodeQuickResultsMessage and decodeQuickResultsMessage handle proper payloads with empty bitmap', () => {
  const decoded = decodeQuickResultsMessage(
    constructPrefixedMessage(
      QR_MESSAGE_FORMAT,
      encodeQuickResultsMessage({
...
test('close_polls message includes bitmap in field 5', async () => {
  const signedQuickResultsReportingUrls =
    await generateSignedQuickResultsReportingUrl(
      {
        electionDefinition,
  ...
  expect(signedQuickResultsReportingUrls).toHaveLength(1);
  const url = new URL(signedQuickResultsReportingUrls[0] as string);
  const payload = url.searchParams.get('p') ?? '';
  const decoded = decodeQuickResultsMessage(payload, election);
  // bitmap is decoded to precinctIds (mocked getPrecinctIdsFromBitmap returns [])
  expect(decoded.precinctIds).toEqual([]);
});

test('non-close_polls message includes bitmap in field 5', async () => {
  const signedQuickResultsReportingUrls =
    await generateSignedQuickResultsReportingUrl(
      {
        electionDefinition,
