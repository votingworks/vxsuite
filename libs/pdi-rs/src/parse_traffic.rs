...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (index, result) in rdr.deserialize().enumerate() {
        ...
        match protocol::parse_packet(&packet.data.0) {
            Ok(([], Packet::Incoming(Incoming::Event(Event::BeginScan)))) => {
                println!("{endpoint} {event:?}", event = Event::BeginScan);
                raw_image_data = RawImageData::new();
            }
            Ok(([], Packet::Incoming(Incoming::Event(Event::EndScan)))) => {
                println!("{endpoint} {event:?}", event = Event::EndScan);
