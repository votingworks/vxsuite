...
fn main_pdictl() -> color_eyre::Result<()> {
    ...
    loop {
        ...
        if let Some(client) = &mut client {
            match client.await_event(Duration::from_millis(10)) {
                Ok(Event::BeginScan) => {
                    println!("begin scan");
                }
                Ok(Event::EndScan) => {
            ...
            if let Some(raw_image_data) = client.try_recv_image_data() {
