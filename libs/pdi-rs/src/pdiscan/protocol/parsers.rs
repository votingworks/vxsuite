...
use super::{
    ...
    types::{
        ...
        DoubleFeedDetectionMode, Resolution, Side,
    },
    Event, Outgoing, Response, Settings, Status, Version,
};
...
pub fn any_incoming(input: &[u8]) -> IResult<&[u8], Incoming> {
    alt((
        map(any_event, Incoming::Event),
        map(any_response, Incoming::Response),
    ))(input)
}
...
fn any_response(input: &[u8]) -> IResult<&[u8], Response> {
    alt((
        map(get_test_string_response, |test_string| {
            Response::GetTestString(test_string.to_owned())
        }),
        map(get_firmware_version_response, Response::GetFirmwareVersion),
        map(
            get_current_firmware_build_version_string_response,
            |version| Response::GetCurrentFirmwareBuildVersionString(version.to_owned()),
        ),
        map(get_scanner_status_response, Response::GetScannerStatus),
        map(get_scanner_settings_response, Response::GetScannerSettings),
        map(get_set_serial_number_response, Response::GetSetSerialNumber),
        map(
        ...
        map(
            ...
            |(side, percent_white_threshold)| Response::AdjustBitonalThreshold {
                side,
        ...
        map(
            ...
            |(white_calibration_table, black_calibration_table)| {
                Response::GetCalibrationInformation {
        ...
        map(
            ...
            Response::GetDoubleFeedDetectionLedIntensity,
        ),
    ))(input)
}
...
fn any_event(input: &[u8]) -> IResult<&[u8], Event> {
    alt((
        alt((
            value(Event::ScannerOkay, scanner_okay_event),
            value(Event::DocumentJam, document_jam_event),
            value(Event::CalibrationNeeded, calibration_needed_event),
            value(Event::ScannerCommandError, scanner_command_error_event),
            value(Event::ReadError, read_error_event),
            value(Event::MsdNeedsCalibration, msd_needs_calibration_event),
            value(
                Event::MsdNotFoundOrOldFirmware,
                msd_not_found_or_old_firmware_event,
            ),
            value(Event::FifoOverflow, fifo_overflow_event),
            value(Event::CoverOpen, cover_open_event),
            value(Event::CoverClosed, cover_closed_event),
            value(Event::CommandPacketCrcError, command_packet_crc_error_event),
        )),
        alt((
            value(Event::FpgaOutOfDate, fpga_out_of_date_event),
            value(Event::CalibrationOk, calibration_ok_event),
            value(
                Event::CalibrationShortCalibrationDocument,
                calibration_short_calibration_document_event,
            ),
            value(
                Event::CalibrationDocumentRemoved,
                calibration_document_removed_event,
            ),
            value(
                Event::CalibrationPixelErrorFrontArrayBlack,
                calibration_pixel_error_front_array_black,
            ),
            value(
                Event::CalibrationPixelErrorFrontArrayWhite,
                calibration_pixel_error_front_array_white,
            ),
            value(Event::CalibrationTimeoutError, calibration_timeout_error),
            value(
                Event::CalibrationSpeedValueError,
                calibration_speed_value_error,
            ),
            value(Event::CalibrationSpeedBoxError, calibration_speed_box_error),
            value(Event::BeginScan, begin_scan_event),
            value(Event::EndScan, end_scan_event),
            value(Event::DoubleFeed, double_feed_event),
        )),
        alt((
            value(Event::CoverOpen, cover_open_event_alternate),
            value(Event::CoverClosed, cover_closed_event_alternate),
        )),
        alt((
            value(
...
/// Parses a request to set the current double feed detection mode.
///
/// # Errors
///
/// Returns an error if the input does not match the expected format.
pub fn set_double_feed_detection_mode_request(
    input: &[u8],
) -> IResult<&[u8], DoubleFeedDetectionMode> {
    map_res(packet_with_crc((tag(b"n1"), decimal_digit)), |(_, mode)| {
        mode.try_into()
    })(input)
}
