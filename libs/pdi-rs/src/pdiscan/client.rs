...
use crate::pdiscan::protocol::{
    image::RawImageData,
    packets::Command,
    parsers,
    types::{
        ClampedPercentage, ColorMode, Direction, DoubleFeedDetectionCalibrationType, EjectMotion,
        Resolution, ScanSideMode, Settings, Side, Speed, Status, Version,
    },
    Incoming, Response,
};
...
use super::{
    protocol::{self, types::DoubleFeedDetectionMode},
    transfer,
};
...
pub struct Client {
    ...
    transfer_event_rx: mpsc::Receiver<transfer::Event>,

    pending_responses: Vec<Response>,
    pending_events: VecDeque<protocol::Event>,

    protocol_event_rx: mpsc::Receiver<protocol::Event>,
    protocol_event_tx: mpsc::Sender<protocol::Event>,
...
impl Client {
    ...
    pub fn open() -> Result<Self> {
        ...
        let (transfer_event_tx, transfer_event_rx) = mpsc::channel();
        let (protocol_event_tx, protocol_event_rx) = mpsc::channel();
        let mut client = Self {
            _device: device,
    ...
    pub fn get_test_string(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetTestString(test_string) => return Ok(test_string),
    ...
    pub fn get_firmware_version(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetFirmwareVersion(version) => return Ok(version),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_scanner_status(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetScannerStatus(status) => return Ok(status),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_scanner_settings(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetScannerSettings(settings) => return Ok(settings),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_required_input_sensors(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetSetRequiredInputSensors {
    ...
    /// Sets the mode of the double feed detection. This should be called before
    /// calibration. The mode parameter specifies the mode to use.
    pub fn set_double_feed_detection_mode(&mut self, mode: DoubleFeedDetectionMode) {
        let mut body = b"n1".to_vec();
        body.extend_from_slice(u8::from(mode).to_string().as_bytes());
        self.validate_and_send_command_unchecked(
            body.as_slice(),
            parsers::set_double_feed_detection_mode_request,
        );
    }
    ...
    pub fn calibrate_double_feed_detection(
        ...
        match self.await_event_matching(
            |event| {
                matches!(
                    event,
                    protocol::Event::DoubleFeedCalibrationComplete
                        | protocol::Event::DoubleFeedCalibrationTimedOut
                )
            },
            deadline.map(|deadline| deadline - Instant::now()),
        )? {
            protocol::Event::DoubleFeedCalibrationComplete => Ok(()),
            protocol::Event::DoubleFeedCalibrationTimedOut => {
                Err(Error::RecvTimeout(RecvTimeoutError::Timeout))
            }
            _ => unreachable!(),
        }
    }
    ...
    pub fn get_double_feed_detection_led_intensity(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetDoubleFeedDetectionLedIntensity(intensity) => return Ok(intensity),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_double_feed_detection_single_sheet_calibration_value(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetDoubleFeedDetectionSingleSheetCalibrationValue(value) => return Ok(value),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_double_feed_detection_double_sheet_calibration_value(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetDoubleFeedDetectionDoubleSheetCalibrationValue(value) => return Ok(value),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_double_feed_detection_double_sheet_threshold_value(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetDoubleFeedDetectionDoubleSheetThresholdValue(value) => return Ok(value),
                self.pending_responses,
            );
        }
    }
    ...
    pub fn get_serial_number(
        ...
        loop {
            ...
            extract_needle!(
                Response::GetSetSerialNumber(serial_number) =>
    ...
    pub fn adjust_bitonal_threshold_by_1(
        ...
        loop {
            ...
            if let Some(Response::AdjustBitonalThreshold {
    ...
    fn get_pending_adjust_bitonal_threshold_response(&mut self, side: Side) -> Option<Response> {
        let mut response: Option<Response> = None;

        extract_needle!(
            needle @ Response::AdjustBitonalThreshold { .. } => response = Some(needle),
            self.pending_responses,
        );

        match response {
            Some(Response::AdjustBitonalThreshold {
                side: side_response,
                ..
    ...
    pub fn try_recv_image_data(&mut self) -> Option<RawImageData> {
        self.raw_image_data.take()
    }
    ...
    pub fn await_event(
        ...
        loop {
            ...
            match self.protocol_event_rx.try_recv() {
                Ok(event) => {
                    return Ok(event);
                }
                Err(TryRecvError::Empty) => {}
                Err(TryRecvError::Disconnected) => {
                    return Err(Error::RecvTimeout(RecvTimeoutError::Disconnected))
                }
            }
        }
    }

    fn await_event_matching(
        &mut self,
        predicate: impl Fn(&protocol::Event) -> bool,
        timeout: impl Into<Option<std::time::Duration>>,
    ) -> Result<protocol::Event> {
        let timeout = timeout.into();
        let deadline = timeout.map(|timeout| Instant::now() + timeout);
        let mut unhandled_events = VecDeque::new();

        let result;
        loop {
            let event = self.await_event(deadline.map(|deadline| deadline - Instant::now()))?;
            ...
            unhandled_events.push_front(event);
        }
        ...
        if self.pending_events.len() > MAX_PENDING_EVENTS {
            tracing::warn!("capping pending events list: {}", self.pending_events.len());

            self.pending_events
                .drain(MAX_PENDING_EVENTS..)
                .for_each(|event| {
                    tracing::warn!("discarding pending event: {event:?}");
                });
        };
    ...
    fn await_incoming(&mut self, deadline: impl Into<Option<std::time::Instant>>) -> Result<()> {
        let transfer_event = match deadline.into() {
            Some(deadline) => self.transfer_event_rx.recv_timeout(
                deadline
                    .checked_duration_since(Instant::now())
                    .unwrap_or_default(),
            )?,
            None => self.transfer_event_rx.recv().unwrap(),
        };

        if transfer_event.is_out() {
            return Ok(());
        }

        match transfer_event {
            transfer::Event::Completed { endpoint, data } => {
            ...
            transfer::Event::Cancelled { endpoint } => {
    ...
    fn handle_incoming(&mut self, incoming: Incoming) {
        ...
        match incoming {
            Incoming::Event(event) => {
                self.protocol_event_tx.send(event).unwrap();
                if matches!(event, protocol::Event::BeginScan) {
