    ...
    if event::poll(Duration::from_millis(50))? {
        if let Key(key) = event::read()? {
            ...
            match (
                ...
                (KeyModifiers::NONE, Char('q'), _, _, _) => app.set_should_quit(),
                (
                    KeyModifiers::NONE,
                    Char('s'),
                    ...
                    _,
                ) => match app.set_feeder_mode(FeederMode::AutoScanSheets) {
                    Ok(_) => {
                    ...
                    Err(e) => app.log(format!("⚠️ Error enabling auto-scan: {e}")),
                },
                (
                    KeyModifiers::NONE,
                    Char('s'),
                    ...
                    _,
                ) => match app.set_feeder_mode(FeederMode::Disabled) {
                    Ok(_) => {
                ...
                (KeyModifiers::SHIFT, Char('R'), ConnectionState::Connected, _, _) => {
