...
After running the autofixer for `vx/gts-no-for-in-loop`, which replaces `for-in`
loops with `for-of` loops, there may be residual `hasOwnProperty` checks that
are inside of `for-of` loops. This rule flags them for manual removal.
