 ...
 */
const ORDERED_BMD_BALLOT_LAYOUTS: Readonly<
    ...
    <React.Fragment>
      ...
      <EnglishTextWrapper>
        <SecondaryLanguageText>
          <BackendInEnglish>{children}</BackendInEnglish>
        </SecondaryLanguageText>
      </EnglishTextWrapper>
    </React.Fragment>
  );
  ...
  return (
    <VoteLine>
      <Font weight="light">
        ...
        >
          [{backendAppStrings.noteBallotContestNoSelection()}]
        </DualLanguageText>
      </Font>
    </VoteLine>
  );
        ...
        <VoteLine key={candidate.id}>
          <Font weight="bold">
            {candidate.isWriteIn ? (
              ...
              <BackendInEnglish>
                {backendElectionStrings.candidateName(candidate)}
              </BackendInEnglish>
            )}
          </Font>{' '}
          {layout.hideParties
            ...
            : candidate.partyIds && (
                <CandidatePartyList
                  ...
                  electionStringsOverride={backendElectionStrings}
                />
              )}
          {candidate.isWriteIn && (
            <DualLanguageText
            ...
            >
              {backendAppStrings.labelWriteInParenthesized()}
            </DualLanguageText>
          )}
        </VoteLine>
      ))}
      {remainingChoices > 0 && (
        <VoteLine>
          <Font weight="light">
            ...
            >
              <InlineBlockSpan>
                [{backendAppStrings.labelNumVotesUnused()}{' '}
                <BackendNumberString value={remainingChoices} />]
              </InlineBlockSpan>
            </DualLanguageText>
          </Font>
        </VoteLine>
  ...
  return (
    <VoteLine>
      <Font weight="bold">
        ...
        >
          {backendElectionStrings.contestOptionLabel(option)}
        </DualLanguageText>
      </Font>
    </VoteLine>
  );
...
interface BmdPaperBallotComponentProps {
 ...
 */
function BmdPaperBallotComponent({
  ballotStyleId,
  electionDefinition,
  ...
  votes,
  machineType,
}: BmdPaperBallotComponentProps): JSX.Element {
  ...
  return withPrintTheme(
    <BackendLanguageOverride languageCode={primaryBallotLanguage}>
        ...
        <Header layout={layout} data-testid="header">
          ...
          <div className="ballot-header-content">
            <H4>
              ...
              >
                {isLiveMode
                  ? backendAppStrings.titleOfficialBallot()
                  : backendAppStrings.titleUnofficialTestBallot()}
              </DualLanguageText>
            </H4>
            <H5>
              <DualLanguageText
              ...
              >
                <PrimaryElectionTitlePrefix
                  ...
                  electionStringsOverride={backendElectionStrings}
                />
                {backendElectionStrings.electionTitle(election)}
              </DualLanguageText>
            </H5>
            <P>
              <DualLanguageText
                ...
                englishTextWrapper={AdjacentTextWithSeparator}
              >
                {backendElectionStrings.electionDate(election)}
              </DualLanguageText>
              <br />
              {backendElectionStrings.countyName(county)},{' '}
              {backendElectionStrings.stateName(election)}
            </P>
          </div>
          <QrCodeContainer>
            ...
            <div>
              <div>
                <div>
                  <div>
                    <BackendInEnglish>
                      {backendAppStrings.titlePrecinct()}
                    </BackendInEnglish>
                  </div>
                  <strong>
                    <BackendInEnglish>
                      {backendElectionStrings.precinctName(precinct)}
                    </BackendInEnglish>
                  </strong>
                </div>
                <div>
                  <div>
                    <BackendInEnglish>
                      {backendAppStrings.titleBallotStyle()}
                    </BackendInEnglish>
                  </div>
                ...
                <div>
                  <div>
                    <BackendInEnglish>
                      {backendAppStrings.titleBallotId()}
                    </BackendInEnglish>
                  </div>
            ...
            {contests.map((contest) => (
              <Contest key={contest.id}>
                <ContestTitle>
                  ...
                  >
                    <InlineBlockSpan>
                      {backendElectionStrings.contestTitle(contest)}
                    </InlineBlockSpan>
                  </DualLanguageText>
                </ContestTitle>
...
export function BackendBmdPaperBallot(
  props: BmdPaperBallotComponentProps & {
    initialI18nStore: Resource;
    languageCode: LanguageCode;
  }
): JSX.Element {
  const { initialI18nStore, languageCode } = props;
  useSSR(initialI18nStore, languageCode);
  ...
  return (
    <FixedLanguageContext.Provider
      value={{
        ...
        i18next,
        translationFunction: i18next.t.bind(i18next),
      }}
    >
      {' '}
      <BmdPaperBallotComponent {...props} />
    </FixedLanguageContext.Provider>
  );
}
