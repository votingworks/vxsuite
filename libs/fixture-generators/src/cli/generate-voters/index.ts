...
function generateVoter(id: number): Record<string, string> {
  return {
    ...
    'Last Name': getRandomElement(lastNames),
    Suffix: '',
    'First Name': getRandomElement(firstNames),
    'Middle Name': getRandomElement(firstNames),
    'Street Number': (Math.floor(Math.random() * 9999) + 1).toString(),
    'Address Suffix': '',
    'House Fraction Number': '',
    'Street Name': getRandomElement(streetNames),
