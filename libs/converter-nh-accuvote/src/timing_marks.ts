...
import { CompleteTimingMarks, Rect } from '@votingworks/ballot-interpreter-nh';
import { Point, PossibleOptionBubblesGrid } from './types';
...
export function computeTimingMarkGrid(
  ...
  const rotated =
    completeTimingMarks.bottomLeftCorner.y <
    completeTimingMarks.topLeftCorner.y;
  ...
  const yComparator: (a: Rect, b: Rect) => number = rotated
    ? (a, b) => b.top - a.top
    : (a, b) => a.top - b.top;
  const top = [...completeTimingMarks.topRects].sort(xComparator);
  const bottom = [...completeTimingMarks.bottomRects].sort(xComparator);
  const left = [...completeTimingMarks.leftRects].sort(yComparator);
  const right = [...completeTimingMarks.rightRects].sort(yComparator);
  assert(
