...
/**
 * Fits text within the given size, adjusting the font size as necessary to fit
 * the text within the size. Tries to use the largest font size that will fit
 * the text within the size. If even the smallest font size is too large, the
 * text will be truncated and an ellipsis will be added.
 *
 ...
 */
function fitTextWithinSize({
  text,
  textConfig: { font, minFontSize, maxFontSize },
  size,
}: {
  text: string;
...
function addContestOptionAnnotationsToPdfPage({
  ...
  optionTextConfig: TextConfig;
  contestTextConfig: TextConfig;
}): void {
  const pageSize = newPdfSize(page.getWidth(), page.getHeight());
  const timingMarkTemplateSize = grid.geometry.timingMarkSize as ImageSize;
  const timingMarkPdfSize = imageSizeToPdfSize(
...
export async function addBallotProofingAnnotationsToPdf(
  ...
  const helveticaBold = pdf.embedStandardFont(StandardFonts.HelveticaBold);
  const helvetica = pdf.embedStandardFont(StandardFonts.Helvetica);
  ...
  for (const [pageNumber, [templateGridSide, page]] of iter(templateGrid)
    ...
    const optionTextConfig: TextConfig = {
      font: helveticaBold,
      minFontSize: 4,
      maxFontSize: 8,
    };

    const contestTextConfig: TextConfig = {
      font: helvetica,
      minFontSize: 3,
      maxFontSize: 7,
    };
