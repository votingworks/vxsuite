...
describe('createUsbDriveAdapter', () => {
  ...
  describe('mounted', () => {
    test('returns ok with mountPoint and sync when drive is mounted', async () => {
      const { multiUsbDrive, assertComplete } = createMockMultiUsbDrive();
      const adapter = createUsbDriveAdapter(multiUsbDrive, () => '/dev/sdb');
      multiUsbDrive.getDrives.reset();
      ...
      const result = await adapter.mounted();
      const mounted = result.unsafeUnwrap();
      expect(mounted.mountPoint).toEqual('/media/vx/usb-drive-sdb1');
    ...
    test('returns err with status when drive is not mounted', async () => {
      const { multiUsbDrive } = createMockMultiUsbDrive();
      const adapter = createUsbDriveAdapter(multiUsbDrive, () => undefined);
      multiUsbDrive.getDrives.reset();
      ...
      expect(await adapter.mounted()).toEqual(err({ status: 'no_drive' }));
    });

    test('returns err with ejected status when drive is ejected', async () => {
      const { multiUsbDrive } = createMockMultiUsbDrive();
      const adapter = createUsbDriveAdapter(multiUsbDrive, () => '/dev/sdb');
      multiUsbDrive.getDrives.reset();
      ...
      expect(await adapter.mounted()).toEqual(err({ status: 'ejected' }));
    });
  });
