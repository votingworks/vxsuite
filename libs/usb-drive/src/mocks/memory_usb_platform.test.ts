...
import { MemoryUsbController } from './memory_usb_platform';
import { UsbDiskDeviceInfo } from '../block_devices';

async function readTestFile(controller: MemoryUsbController): Promise<string> {
...
async function writeTestFile(
  controller: MemoryUsbController,
  data: string
): Promise<void> {
  await writeFile(
    join(controller.getMainPartitionMountPoint(), 'README'),
...
test('drives initially empty', async () => {
  const controller = new MemoryUsbController(makeTemporaryDirectory());
