...
impl BallotGridCandidateMarks {
    ...
    pub fn from_shapes(ballot_page: &BallotPage, shapes: BallotGridBorderShapes) -> Self {
        ...
        shapes.par_map_edgewise(|shapes| {
            shapes
                .par_iter()
                .map(|shape| shape.to_candidate_timing_mark(ballot_page))
                .collect()
        })
    }
